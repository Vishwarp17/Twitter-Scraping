...
def push_to_mongo(pd_tweets):
    ...
    dv = client.twitter_data
    collection = dv.id_data
...
def get_date_input():
    ...
    if (start_date and end_date) and (start_date <= end_date):
        return str(start_date.year) + "-" + str(start_date.month) + "-" + str(start_date.day), str(
            end_date.year) + "-" + str(end_date.month) + "-" + str(end_date.day)
    else:
        st.markdown(
            '<p style="font-weight:bold;color:red;">Error: End date must be either on the same start day or fall after start date.</p>',
...
def scrape_twitter(search_text, date_range=None, max_tweet=None):
    ...
    for i, tweet in enumerate(twts):
        ...
        attributes_container.append(
            [tweet.date, tweet.id, tweet.url, tweet.content, tweet.username, tweet.replyCount, tweet.retweetCount,
             tweet.lang, tweet.sourceLabel, tweet.likeCount])
    tweets_df = pd.DataFrame(attributes_container,
                             columns=["Date Created", "Id", "URL", "Tweet", "User", "Reply Count", "Retweet Count",
...
def main():
    ...
    if st.button("Push data to mongodb"):
        ...
        if pushed_to_mongo.acknowledged:
            ...
            st.markdown('<p style="font-weight:bold;color:red;">Error: Data not pushed to mongodb</p>',
                        unsafe_allow_html=True)
